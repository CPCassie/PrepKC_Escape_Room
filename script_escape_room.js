...
const textContent = {
    en: {
        ...
        solve_instructions: "Using the clues you have received, solve the phrase hidden in the pictogram to escape by matching the letter clues to the picture!",
    ...
    es: {
        ...
        solve_instructions: "Usando las pistas que has recibido, resuelve la frase oculta en el pictograma para escapar, ¡haciendo coincidir las pistas de letras con la imagen!",
...
function updateAnswerKeyDisplay() {
    const keyContent = document.getElementById('key-content');
    const lang = textContent[currentLanguage];

    // Get all completed challenges/clues
    const clueEntries = Object.keys(quizDataEn).map(key => ({
        challengeNum: key,
        clue: quizDataEn[key].clue.letter,
        pet: quizDataEn[key].clue.pet,
        isSolved: collectedClues[quizDataEn[key].clue.pet] !== undefined
    }));

    let html = `<h4>${lang.challenge_title} Status:</h4><ul>`;

    clueEntries.forEach(entry => {
        const status = entry.isSolved ? '✅' : '❌';
        const letter = entry.isSolved ? collectedClues[entry.pet] : '—';
        html += `<li style="list-style-type:none; margin-bottom: 5px;">${status} **Q${entry.challengeNum}** (${entry.pet}): <span style="font-size:1.1em; color:yellow;">${letter}</span></li>`;
    });

    html += `</ul>`;

    // Add final answer area
    const solvedPhraseDisplay = CORRECT_SOLVE_PHRASE.replace(/\s/g, '');
    const currentClueString = finalBoardPets.map(pet => collectedClues[pet] || '_').join('');

    html += `<hr><h4>Final Phrase Clues:</h4><p style="font-size:1.2em; font-family:monospace; color:yellow;">${currentClueString}</p>`;
    
    // Check if solved (only accurate after checkFinalAnswer runs)
    if (document.getElementById('solve-btn') && document.getElementById('solve-btn').disabled) {
        html += `<h4>Status: Solved!</h4><p style="color:#00ff00;">${CORRECT_SOLVE_PHRASE}</p>`;
    }
...
function submitAnswer() {
    const challengeData = currentQuizData[currentChallenge]; // Use currentQuizData
    ...
    if (selectedOption.value === currentQData.correctOption) {
        ...
        if (challengeData.currentQIndex >= totalRemedialQuestions) {
            challengeData.currentQIndex = 0;
            feedbackText.textContent = lang.feedback_incorrect_loop; // Use translated text
        } else {
            feedbackText.textContent = lang.feedback_incorrect; // Use translated text
        }
...
function loadSolveScreen() {
    ...
    solveContainer.innerHTML = `
        <div class="final-clue-images">
            ...
            <img src="CLUES.png" alt="Pet to Letter Clue Map" class="clue-map-image">

            <img src="solve.png" alt="Final Escape Room Puzzle Board" class="solve-board">
        </div>
    `;
    
    // Ensure the input field placeholder is accurate
